...
# pool1 = subset(k99, k99$`Pool ID` == 1 )
# 
# pool1 = subset(pool1, pool1$Group == "Preclinical" | pool1$Group == "Early Preclinical" | pool1$Group == "Control") 
# 
# pool1 = pool1[,c(67,66,63,69)]
# 
# 
# write.table(pool1, "id_pool1.tsv", sep = "\t", row.names = F)
...
saveRDS(dds, "K99ddsADPreclinicalandControls_sex_age_notion")
...
genesdegk99v = genesdegk99$V1


genesdegk99com = subset(genesdegk99, genesdegk99$V1 %in% genesdegv)
...
nombresa = as.data.frame(a$names)


saveRDS(clean.dds, "K99ddsADPreclinicalandControlsqc_sex_agep_notion") #
...
# summary(res)
# 
# write.table (res, "k99adqALLPSvsCODEGallgenes.tsv", quote=FALSE, col.names=T, row.names=T, sep="\t")
# 
# 
# resp = subset(res, res$pvalue < 0.05)
# 
# write.table (resp, "k99adqALLPSvsCODEGraw.tsv", quote=FALSE, col.names=T, row.names=T, sep="\t")
# 
# 
# write.table (res, "k99adqALLPSvsCOallgenes.tsv", quote=FALSE, col.names=T, row.names=T, sep="\t")
# 
# 
# resp = subset(res, res$padj < 0.1)
# 
# write.table (resp, "k99adqALLPSvsCODEG01.tsv", quote=FALSE, col.names=T, row.names=T, sep="\t")
# 
...
saveRDS(rld, "rlogk99qcADPreclinicalandControls_sex_agep_notion")
...
saveRDS(data.pheno, file = "matrix_rlogk99qcADPreclinicalandControls_sex_agep_notion")
...
rld = readRDS("rlogk99qcADPreclinicalandControls_sex_agep_notion")
...
png(file="pca_coding_k99presimnotion.png", width = 2000, height = 1000, units = "px", pointsize = 12,
    res = 100)
...
c2 <- c(
  ...
  png(file="pca_status_k99_presimnotion.png", width = 1000, height = 1000, units = "px", pointsize = 12,
      res = 100)
