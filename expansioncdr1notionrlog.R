...
pheno.expansion.A$Status [pheno.expansion.A$Group=="CDR1"] <- "CA"
...
pheno.expansion.A$Status [pheno.expansion.A$Group=="CDR05"] <- "CDR"
...
saveRDS(dds_phase, "ddsallCDR1old38notion")
...
dds_phase = readRDS("ddsallCDR1old38notion")
...
resdfcdr1 = as.data.frame(res)

write.table (res, "ExpansionCDR1notion.tsv", quote=FALSE, col.names=T, row.names=T, sep="\t")


# countsmatri <- as.data.frame(assay(clean.dds2))
# Counts have negative values
# summary(countsmatri)
# tdatamatri <- as.data.frame(t(countsmatri)

 plotDispEsts(clean.dds)
boxplot(log10(assays(clean.dds)[["cooks"]]), range=0, las=2) # 68031


# write.table (res, "ExpansionALLCDR1vsCOallgenes.tsv", quote=FALSE, col.names=T, row.names=T, sep="\t")
...
saveRDS(clean.dds2, "ddsallCDR1old38qcnotion")
...
saveRDS(rld, "rlogallCDR1old38notion")
...
saveRDS(data.pheno, file = "rlogmatrix_allCDR1old38notion") #~/cfRNA_Biology/rlogLauraqcwhole





rld = readRDS("rlogallCDR1old38notion")
