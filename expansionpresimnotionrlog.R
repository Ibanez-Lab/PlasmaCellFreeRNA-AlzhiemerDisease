...
pheno.expansion.A$Status [pheno.expansion.A$Group=="CDR05"] <- "CDR05"

pheno.expansion.A = subset(pheno.expansion.A, Status == "PreSymptomatic" | Status == "CO")
...
saveRDS(dds_phase, "ddsallpsold38_notion")
...
write.table (res, "expansionpreclinicals.tsv", quote=FALSE, col.names=T, row.names=T, sep="\t")


genesdeg = fread("expansionpreclinicals.tsv")



genesdeg = subset(genesdeg, genesdeg$V1 %in% genesdegk99v)

genesdeg = subset(genesdeg, genesdeg$pvalue < 0.05)

# genesdeg = subset(genesdeg, genesdeg$V1 %in% "ENSG00000196569.12")


genesdegv = genesdeg$V1
...
saveRDS(clean.dds2, "ddsallpsold38qcp_notion")
...
saveRDS(rld, "rlogallpsold38qcp_notion")
...
saveRDS(data.pheno, file = "rlogmatrix_allpsold38qcp_notion") #~/cfRNA_Biology/rlogLauraqcwhole
